...
function initMap() {
  ...
  var spotControlDiv = document.createElement('div');
  var spotControl = new SpotControl(spotControlDiv, map);
  spotControlDiv.index = 2;
  map.controls[google.maps.ControlPosition.LEFT_BOTTOM].push(spotControlDiv);
}
...
function SpotControl(controlDiv, map) {
  ...
  var controlText = document.createElement('i');
  controlText.classList.add("fa");
  controlText.classList.add("fa-arrow-left");
  controlText.style.fontSize = '24px';
  controlUI.appendChild(controlText);
...
function resetMap() {
  ...
  document.getElementById("back-to-spot").classList.add("hidden");
